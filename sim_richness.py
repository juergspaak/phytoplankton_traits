...
NFD_cols = ["ND,{}".format(i) for i in range(1,6)] \
            +["FD,{}".format(i) for i in range(1,6)] + ["no_NFD"]

EF_cols = ["biovolume,{}".format(i) for i in ["05",25,50, 75, 95]]
columns = ["case","species","fac","period","lux1", "lux2", "r_pig_start",
           "r_pig_equi", "r_spec_equi"] +NFD_cols+EF_cols+ prob_cols +["n_fix"]
...
while timer()-start <3600-(time_for_10):
    ...
    (richness_equi, EF_biovolume, r_pig_equi, r_pig_start, prob_spec, 
            prob_pig, n_fix, ND, FD, no_NFD) = rc.fluctuating_richness(
            present_species, 
    ...
    try:
        ...
        for k,case in enumerate(cases):
            data.iloc[len(cases)*i+k] = [cases[k],str(present_species), facs[i], 
                ...
                *ND[k], *FD[k], no_NFD[k], *EF_biovolume[k], *prob_spec[k],
                *prob_pig[k], n_fix]
    except TypeError:
